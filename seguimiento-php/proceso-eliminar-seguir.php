...
if(isset($_SESSION["user"])){
  //echo"Bienvenido", $_SESSION["user"];
	...
	if($id_usuario['id'] != $usuario_id_url) {header("location: ../siguiendo.php");} else {
		...
		$tienda_usuario_id =  $conexion->real_escape_string(utf8_decode($_REQUEST['tienda_usuario_id'])); //usuario_id de esa tienda (OK)
		$usuario_id = 	      $conexion->real_escape_string(utf8_decode($_REQUEST['id_user'])); //usuario_id que quiere seguir la tienda (OK)

		$query = ("DELETE FROM seguimiento WHERE tienda_usuario_id='$tienda_usuario_id' AND usuario_id='$usuario_id'");
