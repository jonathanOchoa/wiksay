...
if(isset($_SESSION["user"])){
  //echo"Bienvenido", $_SESSION["user"];
	...
	if($id_usuario['id'] != $id_usuario_url) {header("location: ../tienda-vendedor?proceso=nomodificado");} else {
		...
		if (isset($_FILES['imagen_perfil']['tmp_name']) AND !empty($_FILES['imagen_perfil']['tmp_name'])){ 
			$imagen_perfil_tienda = addslashes(file_get_contents($_FILES['imagen_perfil']['tmp_name']));

			$query = ("UPDATE tienda SET imagen_tienda='$imagen_perfil_tienda' WHERE usuario_id='$id'");
