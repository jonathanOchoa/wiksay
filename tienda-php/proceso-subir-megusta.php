	    ...
	    if(utf8_encode($id_usuario['apodo']) != $usuario_emisor_url) {header("location: ../producto.php?id=".$id_producto_url."");} else {
			...
			$existe_producto = $resultado->fetch_assoc();
				...
				// si existe no hagas nada:
					header ("location: ../producto.php?nombre=".$producto_id."");
				}else {
				// si no existe haz esto:
					...
					if ($resultado){
					    $query2 = ("INSERT INTO notificacion (tienda_id, nombre_tienda, producto_id, nombre_producto, usuario_id, nombre_usuario, comentario_id, estado, tipo, fecha_alta ) VALUE ('$tienda_id', '$nombre_tienda', '$producto_id', '$nombre_producto', '$id_user', '$nombre_usuario', '', '0', '2 megusta', '$fecha_actual')");
