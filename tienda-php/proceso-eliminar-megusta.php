...
if(isset($_SESSION["user"])){
  //echo"Bienvenido", $_SESSION["user"];
	...
	if($id_usuario['id'] != $usuario_id_url) {header("location: ../producto.php?id=".$id_producto_url."");} else {
		...
		if ($resultado){
		    $query2 = ("DELETE FROM notificacion WHERE tienda_id='$tienda_id' AND nombre_tienda='$nombre_tienda' AND producto_id='$producto_id' AND usuario_id='$id_user' AND tipo='2 megusta'");
