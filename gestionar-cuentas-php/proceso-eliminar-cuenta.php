...
//hasta aqui es el proceso para eliminar la tienda y empresa, ahora empezare con la de eliminar el usuario

// INICIO: ahora elimino los comentarios de a realizado el usuario en los productos

												$query = ("DELETE FROM comentario WHERE usuario_id='$id'");
												$eliminado_comentario_usuario = $conexion->query($query);// con esto le estamos diciendo que nos almacene los datos
										
												if ($eliminado_comentario_usuario){
// FINAL: ahora elimino los comentarios que a realizado el usuario en los productos
// INICIO: ahora elimino los LIKES que a realizado el usuario en los productos													
													$query = ("DELETE FROM megusta WHERE usuario_id='$id'");
													$eliminado_megusta_usuario = $conexion->query($query);// con esto le estamos diciendo que nos almacene los datos
									
													if ($eliminado_megusta_usuario){
// FINAL: ahora elimino los LIKES que a realizado el usuario en los productos
// INICIO: ahora elimino los SEGUIMIENTO que a realizado el usuario en los productos														
														$query = ("DELETE FROM seguimiento WHERE usuario_id='$id'");
														$eliminado_siguiendo_usuario = $conexion->query($query);// con esto le estamos diciendo que nos almacene los datos
										
														if ($eliminado_siguiendo_usuario){
// FINAL: ahora elimino los SEGUIMIENTO que a realizado el usuario en los productos
// INICIO: ahora elimino los NETWORKING que a realizado el usuario en los productos	
															$query = ("DELETE FROM networking WHERE usuario_id='$id'");
															$eliminado_networking_usuario = $conexion->query($query);// con esto le estamos diciendo que nos almacene los datos
										
															if ($eliminado_networking_usuario){
// FINAL: ahora elimino los NETWORKING que a realizado el usuario en los productos
// INICIO: ahora elimino los USUARIO que a realizado el usuario en los productos	
																$query = ("DELETE FROM usuario WHERE id='$id'");
																$eliminado_usuario_usuario = $conexion->query($query);// con esto le estamos diciendo que nos almacene los datos
									
																if ($eliminado_usuario_usuario){
// FINAL: ahora elimino los USUARIO que a realizado el usuario en los productos	
																	session_start();
                                                                    if(isset($_SESSION["user"])){
	                                                                    session_destroy(); 
	                                                                    header("location: ../index.php?proceso=cuenta_eliminada");
                                                                    }else {
		                                                                   header("location: ../index.php?proceso=cuenta_eliminada");
		                                                                  }
																}else {
										                              header("location: ../eliminar-cuenta.php?15"); // usuario_usuario
										                             }
															}else {
																   header("location: ../eliminar-cuenta.php?14"); // networking_usuario
																   }
													    }else {
										                      header("location: ../eliminar-cuenta.php?13"); // seguimiento_usuario
									                          }
												    }else {
										                  header("location: ../eliminar-cuenta.php?12"); // likes_usuario
									                      }
												}else {
										              header("location: ../eliminar-cuenta.php?11"); // comentario_usuario
									                  }
											}else {
										           header("location: ../eliminar-cuenta.php?10"); // empresa
									               }
									    }else {
										      header("location: ../eliminar-cuenta.php?9"); // tienda
									          }
									}else {
										  header("location: ../eliminar-cuenta.php?8"); // social media
									      }
							    }else {
									   header("location: ../eliminar-cuenta.php?7"); // seguimiento
									  }	
							}else {
							       header("location: ../eliminar-cuenta.php?6"); // notificacion
							      }									
						}else {
							   header("location: ../eliminar-cuenta.php?5"); // productos
							  }			   
					}else{
						 header("location: ../eliminar-cuenta.php?3"); // colores
						 }					   
				}else{
					 header("location: ../eliminar-cuenta.php?2"); // likes
					 }
			}else{
				  header("location: ../eliminar-cuenta.php?1"); // comentarios
				 }	
