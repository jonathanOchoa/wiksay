...
if(isset($_SESSION["user"])){
  //echo"Bienvenido", $_SESSION["user"];
	...
	if (utf8_decode($usuario['apodo']) !== $user_url) {header("location: ../cliente.php");} else {
	    ...
	    if ($usuario['id'] !== $user_id_url) {header("location: ../cliente.php");} else {
			...
			if (!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) === false) {
				...
				mail("wiksaycomunicacion@hotmail.com", "Ayuda", "Enviado por $email: $informacion");
			
				header ('location: ../ayuda.php?proceso=email_enviado');
				
                        }else{header("location: ../ayuda.php?proceso=email_incorrecto");}
